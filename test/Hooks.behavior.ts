...
export function shouldBehaveLikeHooks(): void {
	...
	describe("onFeeRefunded callback", () => {
		it("Should call hooks when open fee is refunded on direct cancel", async function () {
			const MockHook = await ethers.getContractFactory("MockHook")
			const affiliateHook = await MockHook.deploy()
			await affiliateHook.waitForDeployment()
			const systemHook = await MockHook.deploy()
			...
			const [quoteId, amount, partyA, partyB, symbolId, affiliate, feeType, calls] = await affiliateHook.getLastFeeRefundCall()
			expect(quoteId).to.equal(1n)
			expect(amount).to.be.gt(0n)
			expect(partyA).to.equal(await user.getAddress())
			expect(partyB).to.equal(ethers.ZeroAddress)
			expect(symbolId).to.equal(1n)
			expect(affiliate).to.equal(context.accountManager)
			expect(feeType).to.equal(0n)
			expect(calls).to.equal(1n)

			const [, , , , , , , systemCalls] = await systemHook.getLastFeeRefundCall()
			expect(systemCalls).to.equal(1n)
		})

		it("Should call hooks when open fee is refunded on force cancel", async function () {
			const MockHook = await ethers.getContractFactory("MockHook")
			const affiliateHook = await MockHook.deploy()
			await affiliateHook.waitForDeployment()
			const systemHook = await MockHook.deploy()
			...
			expect(await affiliateHook.refundFeeCallCount()).to.equal(1n)
			expect(await systemHook.refundFeeCallCount()).to.equal(1n)
		})

		it("Should call hooks when open fee is reimbursed during partyA pending liquidation", async function () {
			const MockHook = await ethers.getContractFactory("MockHook")
			const affiliateHook = await MockHook.deploy()
			await affiliateHook.waitForDeployment()
			const systemHook = await MockHook.deploy()
		...
		it("Should call hooks when open fee is refunded during partyB liquidation", async function () {
			const MockHook = await ethers.getContractFactory("MockHook")
			const affiliateHook = await MockHook.deploy()
			await affiliateHook.waitForDeployment()
			const systemHook = await MockHook.deploy()
		...
		it("Should call hooks when open fee is reimbursed during clearing-house partyA takeover pending liquidation", async function () {
			const MockHook = await ethers.getContractFactory("MockHook")
			const affiliateHook = await MockHook.deploy()
			await affiliateHook.waitForDeployment()
			const systemHook = await MockHook.deploy()
			...
			await context.controlFacet
				.connect(context.signers.admin)
				.grantRole(await context.signers.liquidator.getAddress(), ethers.keccak256(ethers.toUtf8Bytes("CLEARING_HOUSE_ROLE")))
			...
			await user.liquidateAndSetSymbolPrices([1n], [decimal(8n)], [1n])
			await context.clearingHouseFacet.connect(context.signers.liquidator).takeoverPartyALiquidation(await user.getAddress())
			await context.clearingHouseFacet.connect(context.signers.liquidator).liquidatePendingPositionsForClearingHouse(await user.getAddress(), [])
		...
		it("Should call hooks when open fee is reimbursed during clearing-house cross-partyB pending liquidation for liquidated partyA", async function () {
			const MockHook = await ethers.getContractFactory("MockHook")
			const affiliateHook = await MockHook.deploy()
			await affiliateHook.waitForDeployment()
			const systemHook = await MockHook.deploy()
			...
			await user.liquidateAndSetSymbolPrices([1n], [decimal(8n)], [1n])
			await migratePartyBToCross(context, hedger, [1n, 2n])
			...
			await context.clearingHouseFacet
				.connect(context.signers.liquidator)
				.liquidatePendingPositionsForClearingHouse(await hedger.getAddress(), [await user.getAddress()])
