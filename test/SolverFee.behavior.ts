...
const OPEN_POSITION_WITH_SOLVER_FEE =
	"openPosition(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),(uint256,bytes32)[])"
const FILL_CLOSE_WITH_SOLVER_FEE =
	"fillCloseRequest(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),(uint256,bytes32)[])"
...
const FILL_CLOSE_TO_LIQUIDATION_WITH_MAX_AND_SOLVER_FEE =
	"fillCloseRequestToLiquidation(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),(uint256,bytes32)[])"
...
const OPEN_FEE = 0n
const CLOSE_FEE = 1n
const TAG = ethers.encodeBytes32String("solver-fee-test")
type SolverFeeEntry = { amount: bigint; tag: string }
// Scalar-to-list adapter: most cases charge one fee under the default tag; zero means no fee at all.
const feeArrays = (solverFee: bigint, tag: string = TAG): [SolverFeeEntry[]] => (solverFee === 0n ? [[]] : [[{ amount: solverFee, tag }]])
...
export function shouldBehaveLikeSolverFee(): void {
	...
	it("charges open solver fee atomically from PartyA allocated balance into PartyB balance", async function () {
		...
		await expect(txPromise)
			.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
			.withArgs(quoteId, await user.getAddress(), await hedger.getAddress(), await hedger.getAddress(), 1n, OPEN_FEE, decimal(1n), TAG)
	...
	it("charges close solver fee atomically", async function () {
		...
		await expect(txPromise)
			.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
			.withArgs(quoteId, await user.getAddress(), await hedger.getAddress(), await hedger.getAddress(), 1n, CLOSE_FEE, decimal(1n), TAG)
		const balanceEvents = await getPartyABalanceChangeEvents(await txPromise)
	...
	it("can batch atomic open and solver fee charging through SymmioPartyB", async function () {
		...
		await expect(context.symmioPartyB.connect(context.signers.admin)._call([allocateCall, lockCall, openWithFeeCall]))
			.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
			.withArgs(quoteId, await user.getAddress(), partyBAddress, partyBAddress, 1n, OPEN_FEE, decimal(1n), TAG)
	...
	it("opens and charges solver fees atomically only when PartyA remains solvent after the fee", async function () {
		...
		await expect(
			openWithFee(quoteId, quote.quantity, quote.requestedOpenPrice, await getDummyPairUpnlAndPriceSig(quote.requestedOpenPrice, upnlPartyA, 0n), [
				{ amount: decimal(1n), tag: TAG },
			]),
		)
			.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
	...
	it("models the proportional close solver fee in solver-fee-aware close-to-liquidation", async function () {
		...
		await expect(fillToLiquidationWithFee(quoteId, closePrice, await getDummyPairUpnlAndPriceSig(marketPrice, upnlPartyA, 0n), decimal(1n)))
			.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
			.withArgs(quoteId, await user.getAddress(), await hedger.getAddress(), await hedger.getAddress(), 1n, CLOSE_FEE, expectedFee, TAG)
	...
	it("exposes the fee-aware overloads without standalone open or close charge selectors", async function () {
		...
		const standaloneChargeSelector = ethers.id("chargeSolverFees(uint256,uint8,uint256[],bytes[])").slice(0, 10)
		const parallelArraysOpenSelector = ethers
			.id("openPosition(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),uint256[],bytes32[])")
			.slice(0, 10)
		const parallelArraysCloseSelector = ethers
			.id("fillCloseRequest(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),uint256[],bytes32[])")
			.slice(0, 10)
		const scalarFeeOpenSelector = ethers
			.id("openPosition(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),uint256)")
			.slice(0, 10)
		const scalarFeeCloseSelector = ethers
			.id("fillCloseRequest(uint256,uint256,uint256,(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)),uint256)")
			.slice(0, 10)
		const feelessLockAndOpenSelector = ethers
			.id(
				"lockAndOpenPosition(uint256,uint256,uint256,(bytes,uint256,int256,bytes,(uint256,address,address)),(bytes,uint256,int256,int256,uint256,bytes,(uint256,address,address)))",
			)
			.slice(0, 10)
		...
		expect(await context.diamondLoupeFacet.facetAddress(standaloneChargeSelector)).to.equal(ethers.ZeroAddress)
		expect(await context.diamondLoupeFacet.facetAddress(parallelArraysOpenSelector)).to.equal(ethers.ZeroAddress)
		expect(await context.diamondLoupeFacet.facetAddress(parallelArraysCloseSelector)).to.equal(ethers.ZeroAddress)
		expect(await context.diamondLoupeFacet.facetAddress(scalarFeeOpenSelector)).to.equal(ethers.ZeroAddress)
		expect(await context.diamondLoupeFacet.facetAddress(scalarFeeCloseSelector)).to.equal(ethers.ZeroAddress)
		expect(await context.diamondLoupeFacet.facetAddress(feelessLockAndOpenSelector)).to.equal(ethers.ZeroAddress)
	})
	...
	it("rejects a binding maxFillAmount for fee-aware best-effort closes without charging a fee", async function () {
		...
		await expect(
			fillToLiquidationWithMaxAndFee(quoteId, quoteBefore.quantityToClose - 1n, decimal(1n), await getDummyPairUpnlAndPriceSig(decimal(1n), 0n, 0n), [
				{ amount: decimal(1n), tag: TAG },
			]),
	...
	it("charges a zero fee when maxFillAmount is unbounded because the fee is quoted for maxFillAmount", async function () {
		...
		// Zero-rounded entries are dropped, so no charge and no event happen at all.
		...
		).to.not.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
	...
	it("treats maxQuantity as a best-effort ceiling even when it is too small to reach the overshoot", async function () {
		...
		await fill(quoteId, maxQuantity, closePrice, await getDummyPairUpnlAndPriceSig(marketPrice, upnlPartyA, 0n), [])
	...
	it("uses the capped zero-rate fallback and prorates the fee from maxQuantity", async function () {
		...
		const zeroRateAmount = await fill.staticCall(quoteId, maxQuantity, closePrice, sig, ...feeArrays(solverFee))
		await context.symbolControlFacet.connect(context.signers.admin).setPartyBLiquidationOvershootRate(await hedger.getAddress(), 0n, 10n ** 18n)
		const cappedOvershootAmount = await fill.staticCall(quoteId, maxQuantity, closePrice, sig, ...feeArrays(solverFee))
		...
		await expect(fill(quoteId, maxQuantity, closePrice, sig, ...feeArrays(solverFee)))
	...
	it("preserves fee, overshoot, and fill event order while incrementing both uPnL counters once", async function () {
		...
		const fill = (context.partyBExecutionFacet.connect(hedger.signer) as any)[FILL_CLOSE_TO_LIQUIDATION_WITH_MAX_AND_SOLVER_FEE]
		const receipt = await (
			await fill(quoteId, quote.quantityToClose, closePrice, await getDummyPairUpnlAndPriceSig(marketPrice, upnlPartyA, 0n), ...feeArrays(solverFee))
		).wait()
		...
		const feeIndex = eventNames.indexOf("SolverFeeCharged")
	...
	it("rolls back a precharged fee when Muon validation fails and preserves the bound-mode exemption", async function () {
		...
		const fill = (context.partyBExecutionFacet.connect(hedger.signer) as any)[FILL_CLOSE_TO_LIQUIDATION_WITH_MAX_AND_SOLVER_FEE]
		const args = [quoteId, quote.quantityToClose, decimal(1n), await getDummyPairUpnlAndPriceSig(decimal(1n), 0n, 0n), ...feeArrays(solverFee)]
	...
	it("retains PartyB ownership guards on the direct fee-aware close-to-liquidation path", async function () {
		...
		await expect(fill(quoteId, ethers.MaxUint256, decimal(1n), await getDummyPairUpnlAndPriceSig(decimal(1n), 0n, 0n), [])).to.be.revertedWith(
	...
	it("preserves every low-filled-amount guard at the exact component release boundary", async function () {
		...
		for (const tooSmallAmount of [1n, minimumPassingAmount - 1n]) {
			await expect(fill(quoteId, tooSmallAmount, decimal(1n), sig, [])).to.be.revertedWith("LibQuote: Low filled amount")
		}
		await expect(fill(quoteId, minimumPassingAmount, decimal(1n), sig, [])).to.not.be.reverted
	...
	it("uses the prorated solver fee while planning a fill below maxQuantity", async function () {
		...
		const fill = (context.partyBExecutionFacet.connect(hedger.signer) as any)[FILL_CLOSE_TO_LIQUIDATION_WITH_MAX_AND_SOLVER_FEE]
		await expect(
			fill(quoteId, quote.quantityToClose, closePrice, await getDummyPairUpnlAndPriceSig(marketPrice, upnlPartyA, 0n), ...feeArrays(solverFee)),
		)
			.to.emit(context.partyBExecutionFacet, "SolverFeeCharged")
			.withArgs(quoteId, await user.getAddress(), await hedger.getAddress(), await hedger.getAddress(), 1n, CLOSE_FEE, expectedFee, TAG)
	...
	it("charges a list of OPEN fees to their respective tagged receivers in one call", async function () {
		const partyA = await user.getAddress()
		const partyB = await hedger.getAddress()
		const tagA = ethers.encodeBytes32String("rfq/desk-a")
		const tagB = ethers.encodeBytes32String("rfq/desk-b")
		const receiverA = context.signers.feeCollector.address
		const receiverB = context.signers.liquidator.address
		await (context.controlFacet.connect(hedger.signer) as any).setSolverFeeReceiverForTag(partyB, tagA, receiverA)
		await (context.controlFacet.connect(hedger.signer) as any).setSolverFeeReceiverForTag(partyB, tagB, receiverB)

		const quoteId = await sendQuoteWithSolverFeeCaps(decimal(2n, 16))
		await hedger.lockQuote(quoteId)
		const quote = await context.viewFacetQuote.getQuote(quoteId)
		const partyAAllocatedBefore = (await user.getBalanceInfo()).allocatedBalances
		const receiverABefore = await context.viewFacet.balanceOf(receiverA)
		const receiverBBefore = await context.viewFacet.balanceOf(receiverB)

		const openWithFee = (context.partyBExecutionFacet.connect(hedger.signer) as any)[OPEN_POSITION_WITH_SOLVER_FEE]
		const txPromise = openWithFee(
			quoteId,
			quote.quantity,
			quote.requestedOpenPrice,
			await getDummyPairUpnlAndPriceSig(quote.requestedOpenPrice, 0n, 0n),
			[
				{ amount: decimal(1n), tag: tagA },
				{ amount: decimal(5n, 17), tag: tagB },
			],
		)
		await expect(txPromise)
			.to.emit(context.partyBExecutionFacet, "SolverFeeCharged")
			.withArgs(quoteId, partyA, partyB, receiverA, 1n, OPEN_FEE, decimal(1n), tagA)
		await expect(txPromise)
			.to.emit(context.partyBExecutionFacet, "SolverFeeCharged")
			.withArgs(quoteId, partyA, partyB, receiverB, 1n, OPEN_FEE, decimal(5n, 17), tagB)
		const balanceEvents = await getPartyABalanceChangeEvents(await txPromise)

		expect(await context.viewFacet.balanceOf(receiverA)).to.equal(receiverABefore + decimal(1n))
		expect(await context.viewFacet.balanceOf(receiverB)).to.equal(receiverBBefore + decimal(5n, 17))
		expect(partyAAllocatedBefore - (await user.getBalanceInfo()).allocatedBalances).to.equal(decimal(15n, 17))
		expect((await getSolverFeeState(quoteId)).openFeeCharged).to.equal(decimal(15n, 17))
		expect(
			balanceEvents.filter((event: any) => event.partyA === partyA && event.changeType === BigInt(BalanceChangeType.OPEN_SOLVER_FEE_OUT)).length,
		).to.equal(2)
	})

	it("caps a CLOSE fee list on the batch total and reverts the whole fill when it does not fit", async function () {
		const quoteId = await sendQuoteWithSolverFeeCaps(NO_SOLVER_FEE, "close-list-cap", undefined, decimal(2n, 16))
		await openQuote(quoteId)
		await requestClose(quoteId)
		...
		// Closed notional 100, cap 2. A list summing to 2.1 must reject atomically without filling anything.
		await expect(
			fillWithFee(quoteId, quote.quantityToClose, decimal(1n), sig, [
				{ amount: decimal(15n, 17), tag: TAG },
				{ amount: decimal(6n, 17), tag: TAG },
			]),
		).to.be.revertedWith("SolverFee: Solver fee rate cap exceeded")
		expect((await context.viewFacetQuote.getQuote(quoteId)).closedAmount).to.equal(0n)
		expect((await getSolverFeeState(quoteId)).closeFeeCharged).to.equal(0n)

		await fillWithFee(quoteId, quote.quantityToClose, decimal(1n), sig, [
			{ amount: decimal(15n, 17), tag: TAG },
			{ amount: decimal(5n, 17), tag: TAG },
		])
		expect((await getSolverFeeState(quoteId)).closeFeeCharged).to.equal(decimal(2n))
	})

	it("rejects zero-amount fee entries on the fee-aware overloads", async function () {
		const quoteId = await sendQuoteWithSolverFeeCaps(decimal(2n, 16))
		await hedger.lockQuote(quoteId)
		const quote = await context.viewFacetQuote.getQuote(quoteId)
		const sig = await getDummyPairUpnlAndPriceSig(quote.requestedOpenPrice, 0n, 0n)
		const openWithFee = (context.partyBExecutionFacet.connect(hedger.signer) as any)[OPEN_POSITION_WITH_SOLVER_FEE]

		await expect(
			openWithFee(quoteId, quote.quantity, quote.requestedOpenPrice, sig, [
				{ amount: decimal(1n), tag: TAG },
				{ amount: 0n, tag: TAG },
			]),
		).to.be.revertedWith("SolverFee: Zero amount")
		expect((await getSolverFeeState(quoteId)).openFeeCharged).to.equal(0n)
	})

	it("prorates each fee entry independently and drops zero-rounded entries on close-to-liquidation", async function () {
		const quoteId = await sendQuoteWithSolverFeeCaps(decimal(100n))
		await openQuote(quoteId)
		await requestClose(quoteId)
		...
		const closePrice = decimal(1n)
		const marketPrice = decimal(2n)
		const bigFee = decimal(1n)
		const tinyFee = 1n // floors to zero on any partial fill and must be dropped, not charged
		const balanceInfo = await user.getBalanceInfo()
		const upnlPartyA = bigFee + tinyFee - (balanceInfo.allocatedBalances - balanceInfo.lockedCva - balanceInfo.lockedLf)
		const quote = await context.viewFacetQuote.getQuote(quoteId)
		const tagA = ethers.encodeBytes32String("prorate/kept")
		const tagB = ethers.encodeBytes32String("prorate/dropped")

		const [maxCloseAmount] = await context.viewFacetQuote.getMaxCloseAmountToLiquidation(
			quoteId,
			closePrice,
			marketPrice,
			upnlPartyA,
		...
		expect(maxCloseAmount).to.be.greaterThan(0n)
		expect(maxCloseAmount).to.be.lessThan(quote.quantityToClose)
		const expectedFee = (bigFee * maxCloseAmount) / quote.quantityToClose

		const fill = (context.partyBExecutionFacet.connect(hedger.signer) as any)[FILL_CLOSE_TO_LIQUIDATION_WITH_MAX_AND_SOLVER_FEE]
		const txPromise = fill(quoteId, quote.quantityToClose, closePrice, await getDummyPairUpnlAndPriceSig(marketPrice, upnlPartyA, 0n), [
			{ amount: bigFee, tag: tagA },
			{ amount: tinyFee, tag: tagB },
		])
		await expect(txPromise)
			.to.emit(context.partyBExecutionFacet, "SolverFeeCharged")
			.withArgs(quoteId, await user.getAddress(), await hedger.getAddress(), await hedger.getAddress(), 1n, CLOSE_FEE, expectedFee, tagA)
		const receipt = await (await txPromise).wait()
		const feeEvents = receipt.logs.filter((log: any) => {
			try {
				return context.partyBExecutionFacet.interface.parseLog(log)?.name === "SolverFeeCharged"
			} catch {
				return false
			}
		})
	...
	it("rejects CLOSE fees for a suspended PartyA before any state changes", async function () {
		const quoteId = await sendQuoteWithSolverFeeCaps(decimal(2n, 16))
		await openQuote(quoteId)
		await requestClose(quoteId)
		await context.pauseControlFacet.connect(context.signers.admin).suspendedAddress(await user.getAddress())

		await expect(fillCloseWithFees(quoteId, decimal(1n))).to.be.revertedWith("SolverFee: Payer suspended")
		expect((await getSolverFeeState(quoteId)).closeFeeCharged).to.equal(0n)
		expect((await context.viewFacetQuote.getQuote(quoteId)).closedAmount).to.equal(0n)
	})
	...
	describe("Solver fee receiver", () => {
		...
		it("credits the configured receiver on open; PartyB free balance stays flat", async function () {
			...
			await expect(openQuoteWithFees(quoteId, decimal(1n)))
				.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
				.withArgs(quoteId, await user.getAddress(), partyB, receiver, 1n, OPEN_FEE, decimal(1n), TAG)
		...
		it("credits the configured receiver on close", async function () {
			...
			await expect(fillCloseWithFees(quoteId, decimal(1n)))
				.to.emit(context.partyBQuoteActionsFacet, "SolverFeeCharged")
				.withArgs(quoteId, await user.getAddress(), partyB, receiver, 1n, CLOSE_FEE, decimal(1n), TAG)
	...
	describe("Tagged receiver routing", () => {
		...
		it("routes each tag independently while keeping one cumulative cap", async function () {
			const partyB = await hedger.getAddress()
			const tagA = ethers.encodeBytes32String("route/a")
			const tagB = ethers.encodeBytes32String("route/b")
			...
			const openWithFee = (context.partyBExecutionFacet.connect(hedger.signer) as any)[OPEN_POSITION_WITH_SOLVER_FEE]
			// Cumulative cap is 2; the two tags share it, so 1 + 1 fits and 1 + 1.5 would not.
			await expect(
				openWithFee(quoteId, quote.quantity, quote.requestedOpenPrice, await getDummyPairUpnlAndPriceSig(quote.requestedOpenPrice, 0n, 0n), [
					{ amount: decimal(1n), tag: tagA },
					{ amount: decimal(15n, 17), tag: tagB },
				]),
			).to.be.revertedWith("SolverFee: Solver fee rate cap exceeded")
			await openWithFee(quoteId, quote.quantity, quote.requestedOpenPrice, await getDummyPairUpnlAndPriceSig(quote.requestedOpenPrice, 0n, 0n), [
				{ amount: decimal(1n), tag: tagA },
				{ amount: decimal(1n), tag: tagB },
			])
		...
		it("lets PartyB manager configure a tag and rejects unrelated callers", async function () {
			const partyB = await hedger.getAddress()
			const tag = ethers.encodeBytes32String("managed")
			await expect(
		...
		it("rejects a tagged receiver that routes the fee back to PartyA", async function () {
			const partyB = await hedger.getAddress()
			const tag = ethers.encodeBytes32String("back-to-a")
			await (context.controlFacet.connect(hedger.signer) as any).setSolverFeeReceiverForTag(partyB, tag, await user.getAddress())
			const quoteId = await sendQuoteWithSolverFeeCaps(decimal(2n, 16))
			await hedger.lockQuote(quoteId)
			await expect(openQuoteWithFees(quoteId, decimal(1n), undefined, decimal(1n), 0n, 0n)).to.not.be.reverted // default TAG has no override

			const secondQuoteId = await sendQuoteWithSolverFeeCaps(decimal(2n, 16))
			await hedger.lockQuote(secondQuoteId)
			const quote = await context.viewFacetQuote.getQuote(secondQuoteId)
			const openWithFee = (context.partyBExecutionFacet.connect(hedger.signer) as any)[OPEN_POSITION_WITH_SOLVER_FEE]
			await expect(
				openWithFee(secondQuoteId, quote.quantity, quote.requestedOpenPrice, await getDummyPairUpnlAndPriceSig(quote.requestedOpenPrice, 0n, 0n), [
					{ amount: decimal(1n), tag },
				]),
			).to.be.revertedWith("SolverFee: Receiver is partyA")
		})
	})
}
