...
export function shouldBehaveLikeFundingRate(): void {
	...
	describe("Accumulative Funding Rate Methods", function () {
		describe("setEpochDuration", function () {
			...
			it("should bump both partyA and partyB nonces for affected pairs", async () => {
				const partyA = context.signers.user.address
				const partyB = context.signers.hedger.address

				const beforePartyANonce = await context.viewFacet.nonceOfPartyA(partyA)
				const beforePartyBNonce = await context.viewFacet.nonceOfPartyB(partyB, partyA)

				await context.fundingRateFacet.connect(context.signers.hedger).setEpochDurations([1], [EightHourInSec])

				expect(await context.viewFacet.nonceOfPartyA(partyA)).to.equal(beforePartyANonce + 1n)
				expect(await context.viewFacet.nonceOfPartyB(partyB, partyA)).to.equal(beforePartyBNonce + 1n)
			})

			it("should not bump nonces when epoch duration value is unchanged", async () => {
				const partyA = context.signers.user.address
				const partyB = context.signers.hedger.address
				...
				expect(await context.viewFacet.nonceOfPartyA(partyA)).to.equal(beforePartyANonce)
				expect(await context.viewFacet.nonceOfPartyB(partyB, partyA)).to.equal(beforePartyBNonce)
			})

			it("should not bump nonces for pairs without open positions on updated symbols", async () => {
				const partyA = context.signers.user.address
				const partyB = context.signers.hedger.address
			...
			it("should bump nonces for all affected partyAs of the same partyB", async () => {
				const user2 = new User(context, context.signers.user2)
				await user2.setup()
				await user2.setBalances(decimal(5000n), decimal(5000n), decimal(5000n))

				const user2QuoteId = await user2.sendQuote()
				await hedger.lockQuote(user2QuoteId)
				await hedger.openPosition(user2QuoteId)

				const partyB = context.signers.hedger.address
				const partyA1 = context.signers.user.address
				const partyA2 = context.signers.user2.address

				const beforePartyA1Nonce = await context.viewFacet.nonceOfPartyA(partyA1)
				const beforePartyB1Nonce = await context.viewFacet.nonceOfPartyB(partyB, partyA1)
				const beforePartyA2Nonce = await context.viewFacet.nonceOfPartyA(partyA2)
				const beforePartyB2Nonce = await context.viewFacet.nonceOfPartyB(partyB, partyA2)

				await context.fundingRateFacet.connect(context.signers.hedger).setEpochDurations([1], [EightHourInSec])

				expect(await context.viewFacet.nonceOfPartyA(partyA1)).to.equal(beforePartyA1Nonce + 1n)
				expect(await context.viewFacet.nonceOfPartyB(partyB, partyA1)).to.equal(beforePartyB1Nonce + 1n)
				expect(await context.viewFacet.nonceOfPartyA(partyA2)).to.equal(beforePartyA2Nonce + 1n)
				expect(await context.viewFacet.nonceOfPartyB(partyB, partyA2)).to.equal(beforePartyB2Nonce + 1n)
			})

			it("should bump all connected partyA nonces in cross mode when any changed symbol is active", async () => {
				const user2 = new User(context, context.signers.user2)
				await user2.setup()
				await user2.setBalances(decimal(5000n), decimal(5000n), decimal(5000n))
				...
				const user2QuoteId = await user2.sendQuote(limitQuoteRequestBuilder().symbolId(2).build())
				await hedger.lockQuote(user2QuoteId)
				await hedger.openPosition(user2QuoteId)

				await context.controlFacet.connect(context.signers.admin).setCrossPartyBModeActivated(true)
				await context.controlFacet.connect(context.signers.admin).setCrossPartyB(context.signers.hedger.address, true)
				...
				const beforePartyA1Nonce = await context.viewFacet.nonceOfPartyA(partyA1)
				const beforePartyA2Nonce = await context.viewFacet.nonceOfPartyA(partyA2)
				const beforeCrossNonce = await context.viewFacet.nonceOfPartyB(partyB, ethers.ZeroAddress)

				await context.fundingRateFacet.connect(context.signers.hedger).setEpochDurations([1], [EightHourInSec])

				expect(await context.viewFacet.nonceOfPartyA(partyA1)).to.equal(beforePartyA1Nonce + 1n)
				expect(await context.viewFacet.nonceOfPartyA(partyA2)).to.equal(beforePartyA2Nonce + 1n)
				expect(await context.viewFacet.nonceOfPartyB(partyB, ethers.ZeroAddress)).to.equal(beforeCrossNonce + 2n)
			})

			it("should not bump nonces in cross mode when changed symbols are inactive for partyB", async () => {
				await context.controlFacet.connect(context.signers.admin).setCrossPartyBModeActivated(true)
				await context.controlFacet.connect(context.signers.admin).setCrossPartyB(context.signers.hedger.address, true)
				...
				expect(await context.viewFacet.nonceOfPartyA(partyA)).to.equal(beforePartyANonce)
				expect(await context.viewFacet.nonceOfPartyB(partyB, partyA)).to.equal(beforePartyBNonce)
				expect(await context.viewFacet.nonceOfPartyB(partyB, ethers.ZeroAddress)).to.equal(beforeCrossNonce)
			})
		})
