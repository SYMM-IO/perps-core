...
// setPendingQuotesValidLength is gated by PROTOCOL_LIMITS_ROLE; the test only needs one representative role
	...
	describe("AccessControl role admins", () => {
		...
		describe("unit", () => {
			describe("isRoleAdmin", () => {
				it("Should treat default admin as admin for every role", async function () {
					...
					expect(await context.viewFacet.isRoleAdmin(await admin.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(true)
					expect(await context.viewFacet.isRoleAdmin(await admin.getAddress(), LIQUIDATOR_ROLE)).to.equal(true)
					expect(await context.viewFacet.isRoleAdmin(await outsider.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(false)
				})
			})
			...
			describe("addRoleAdmin", () => {
				it("Should add a new admin for a role", async function () {
					...
					expect(await context.viewFacet.isRoleAdmin(await secondaryAdmin.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(false)
					// add admin as default admin
					await expect(context.controlFacet.connect(admin).addRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress()))
						.to.emit(context.controlFacet, "RoleAdminAdded")
						.withArgs(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress())
					expect(await context.viewFacet.isRoleAdmin(await secondaryAdmin.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(true)
				})

				it("Should revert when caller is not a role admin", async function () {
					// outsider cannot add
					await expect(
						context.controlFacet.connect(outsider).addRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress()),
				...
				it("Should revert when role admin (non-default) tries to add another admin", async function () {
					...
					await context.controlFacet.connect(admin).addRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress())
					// secondaryAdmin cannot further delegate
					await expect(
						context.controlFacet.connect(secondaryAdmin).addRoleAdmin(PROTOCOL_LIMITS_ROLE, await operator.getAddress()),
					).to.be.revertedWith("Accessibility: Must have role")
				})

				it("Should revert when admin address is zero", async function () {
					// prevent zero admin
					await expect(context.controlFacet.connect(admin).addRoleAdmin(PROTOCOL_LIMITS_ROLE, ethers.ZeroAddress)).to.be.revertedWith(
			...
			describe("removeRoleAdmin", () => {
				...
				it("Should remove an existing admin for a role", async function () {
					// remove admin
					await expect(context.controlFacet.connect(admin).removeRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress()))
				...
				it("Should revert when caller is not a role admin", async function () {
					...
					await expect(
						context.controlFacet.connect(outsider).removeRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress()),
					).to.be.revertedWith("Accessibility: Must have role")
				})
				...
				it("Should block delegated admins from removing other delegated admins", async function () {
					...
					await context.controlFacet.connect(admin).addRoleAdmin(PROTOCOL_LIMITS_ROLE, await operator.getAddress())
					await expect(
						context.controlFacet.connect(secondaryAdmin).removeRoleAdmin(PROTOCOL_LIMITS_ROLE, await operator.getAddress()),
					).to.be.revertedWith("Accessibility: Must have role")
				})
				...
				it("Should block delegated admins from removing themselves", async function () {
					...
					await expect(
						context.controlFacet.connect(secondaryAdmin).removeRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress()),
					).to.be.revertedWith("Accessibility: Must have role")
				})
			})
		...
		describe("scenario", () => {
			it("Should handle multi-admin lifecycle across roles", async function () {
				...
				await context.controlFacet.connect(admin).addRoleAdmin(PROTOCOL_LIMITS_ROLE, await operator.getAddress())
				expect(await context.viewFacet.isRoleAdmin(await secondaryAdmin.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(true)
				expect(await context.viewFacet.isRoleAdmin(await operator.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(true)

				// delegated admin grants setter role to outsider
				await context.controlFacet.connect(secondaryAdmin).grantRole(await outsider.getAddress(), PROTOCOL_LIMITS_ROLE)
				expect(await context.viewFacet.hasRole(await outsider.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(true)
				...
				await context.controlFacet.connect(admin).removeRoleAdmin(PROTOCOL_LIMITS_ROLE, await secondaryAdmin.getAddress())
				await expect(context.controlFacet.connect(secondaryAdmin).grantRole(await admin.getAddress(), PROTOCOL_LIMITS_ROLE)).to.be.revertedWith(
					"Accessibility: Must be role admin",
				)

				// remaining admin revokes the operator and cleans up
				await context.controlFacet.connect(operator).revokeRole(await outsider.getAddress(), PROTOCOL_LIMITS_ROLE)
				expect(await context.viewFacet.hasRole(await outsider.getAddress(), PROTOCOL_LIMITS_ROLE)).to.equal(false)
