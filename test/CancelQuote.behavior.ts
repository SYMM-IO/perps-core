	...
	describe("Should cancel a locked quote", async function () {
		...
		describe("Should cancel successfully", async function () {
			...
			it("Should force cancel quote", async function () {
				...
				const lockCountBefore = await context.viewFacet.getPartyALockedQuotesCount(user.address)
				expect(lockCountBefore).to.be.greaterThan(0)
				await time.increase(300)
				await user.forceCancelQuote(1)
				const lockCountAfter = await context.viewFacet.getPartyALockedQuotesCount(user.address)
				expect(lockCountAfter).to.equal(lockCountBefore - 1n)
