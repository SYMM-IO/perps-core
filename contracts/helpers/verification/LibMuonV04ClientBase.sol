...
/// @notice Library for Schnorr signature verification used by the Muon oracle
library LibMuonV04ClientBase {
	...
	/// @notice Validates that a public key x ordinate is less than HALF_Q
	function validatePubKey(uint256 signingPubKeyX) public pure {
		require(signingPubKeyX < HALF_Q, "Public-key x >= HALF_Q");
	}

	/// @notice Verifies a Muon TSS Schnorr signature against a public key
	function muonVerify(
		uint256 hash,
