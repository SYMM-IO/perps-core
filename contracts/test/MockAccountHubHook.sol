...
contract MockAccountHubHook {
    ...
    function onAccountCreation(address user, address account) external returns (bool) {
    ...
    function onVirtualAccountCreation(address virtualAccount, address parent) external returns (bool) {
        ...
        if (recordCalls) {
            hookCalls.push(HookCall({
                selector: selector,
                data: abi.encode(virtualAccount, parent),
                timestamp: block.timestamp,
                callCount: selectorCallCount[selector]
            }));
        }
     ...
     */
    function getHookCall(uint256 index) external view returns (
        bytes4 selector,
        bytes memory data,
        uint256 timestamp,
        uint256 callCount
     ...
     */
    function getAllCallCounts() external view returns (
    ...
    function assertAccountCreationCalled(address account) external view returns (bool) {
        return accountHookCalled[account] && 
               lastAccountForSelector[this.onAccountCreation.selector] == account;
    }
    ...
    function assertVirtualAccountCreationCalled(address virtualAccount) external view returns (bool) {
        return accountHookCalled[virtualAccount] && 
               lastAccountForSelector[this.onVirtualAccountCreation.selector] == virtualAccount;
    }
}