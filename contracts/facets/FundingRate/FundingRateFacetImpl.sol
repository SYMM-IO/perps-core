...
library FundingRateFacetImpl {
	...
	function chargeFundingRate(address partyA, uint256[] memory quoteIds, int256[] memory rates, PairUpnlSig memory upnlSig) internal {
		...
		for (uint256 i = 0; i < quoteIds.length; i++) {
			...
			// Update open position notionals for both PartiesPositionsInfo
			LibQuote.updatePartiesOpenPositionNotional(quote, oldOpenedPrice);
		...
		LibAccount.increasePartyBNonce(signer, partyA);
