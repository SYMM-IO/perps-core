...
contract SymmioFeeDistributor is Initializable, PausableUpgradeable, AccessControlEnumerableUpgradeable {
	...
	error NotStakeholderOrCollector();
	...
	function isStakeholder(address user) internal view returns (bool) {
		uint256 len = stakeholders.length;
		for (uint256 i = 0; i < len; i++) {
			if (stakeholders[i].receiver == user) return true;
		}
		return false;
	}

	/// @notice Claims all available fees and distributes them to stakeholders
	function claimAllFee() external whenNotPaused {
		if (!hasRole(COLLECTOR_ROLE, msg.sender) && !isStakeholder(msg.sender)) revert NotStakeholderOrCollector();
	...
	function claimFee(uint256 amount) public whenNotPaused {
		if (!hasRole(COLLECTOR_ROLE, msg.sender) && !isStakeholder(msg.sender)) revert NotStakeholderOrCollector();
