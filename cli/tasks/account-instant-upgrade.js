...
export const ACCOUNT_INSTANT_PLAN = Object.freeze([
	...
	{ id: "retire-party-b", phase: "execution", title: "Remove old InstantLayer trust through each PartyB administrator" },
...
async function runPhase(ctx, input, phase, { authority, env = {}, fork = false } = {}) {
...
export async function executePartyBStage(ctx, input, retire = false) {
	...
	for (const authority of [...new Set(report.actions.map(action => action.authority))].sort()) {
		const actions = report.actions.filter(action => action.authority === authority);
		ctx.ui.note(actions.map(a => `${a.to}: ${a.description}\nvalue=${a.value}\n${a.data}`).join("\n\n"), `PartyB administrator ${authority}`);
		const role = `party-b-${authority.toLowerCase()}`;
		let signer = ctx.getSigner(role);
		if (!signer) {
			signer = await selectSigner(ctx.ui, {
				role: "PartyB administrator",
			...
			ctx.bindSigner(role, signer);
		}
		validateSignerSelection(signer, { allowSafe: false });
		if (signer.address?.toLowerCase() !== authority.toLowerCase()) throw new Error("PartyB signer differs from the verified administrator");
		await withSignerEnvironment(signer, ctx.ui, () =>
			runPhase(ctx, input, retire ? "execute-retire-party-b" : "execute-party-b", { authority, env: executeEnvironment }),
		);
	}
	if ((await runPhase(ctx, input, phase)).actions.length)
		throw new Error("PartyB transactions did not produce the intended trust and whitelist state");
}
	...
	return common({
		...
		run: async (ctx, input) => {
			...
			await step("authorize", async () => {
				...
				ctx.ui.note(
					`Snapshot: ${configurationPath(input)}\nGasless proxy/storage remain in place. InstantLayer user delegations/replay state are not migrated. No global or per-account timelock setters are called.`,
					"Upgrade scope",
				);
			...
			await step("retire-party-b", () => executePartyBStage(ctx, input, true));
