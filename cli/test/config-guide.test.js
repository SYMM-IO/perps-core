...
test("configuration guide separates recipe inputs from generated deployment state", () => {
	const live = configurationLocations({ chainId: 42161, recipePath: "deployment-recipes/arbitrum.json" });
	assert.match(live.recipeExample, /deployment-tooling\/examples\/arbitrum\.v1\.example\.json$/);
	assert.match(live.recipe, /deployment-recipes\/arbitrum\.json$/);
	...
	const fork = configurationLocations({ chainId: 42161, simulated: true, recipePath: "deployment-recipes/fork-arbitrum.json" });
...
test("help starts from guided recipes and the menu-only checkout-local launcher", () => {
	...
	assert.match(help, /deployment-tooling\/examples\/arbitrum\.v1\.example\.json/);
	assert.match(help, /deployment-recipes\/<name>\.json/);
...
test("recipe diagnostics identify exact editable JSON fields", () => {
	...
	assert.match(rendered.detail, /deployment-recipes\/arbitrum\.json/);
});

test("doctor next steps return operators to guided correction and review", () => {
	const recipe = doctorNextStepLines({
		networkName: "arbitrum",
		recipePath: "/repo/deployment-recipes/prod.json",
		blockingFields: ["governance.admin", "partyB.signer"],
	}).join("\n");
	assert.match(recipe, /guided form for \/repo\/deployment-recipes\/prod\.json \(governance\.admin, partyB\.signer\)/);
	...
	const component = doctorNextStepLines({
		networkName: "arbitrum",
		recipePath: "/repo/deployment-recipes/arbitrum-partyB.json",
		only: "partyB",
	}).join("\n");
	assert.match(component, /guided form for \/repo\/deployment-recipes\/arbitrum-partyB\.json/);
...
test("runtime guide shows the selected recipe, report, and checkpoint destinations", () => {
	const guide = runtimeConfigurationRows({ chainId: 42161, simulated: true, recipePath: "deployment-recipes/fork.json" });
	const rendered = guide.rows.flat().join("\n");
	assert.match(rendered, /deployment-recipes\/fork\.json/);
	...
	const component = runtimeConfigurationRows({
		chainId: 42161,
		simulated: false,
		recipePath: "deployment-recipes/arbitrum-partyB.json",
