...
export async function status(args) {
	...
	// v0.8.6 renamed the getter to `getOwner()`; diamonds deployed before that only expose
	// `owner()`, and status runs against both, so try the new name and fall back to the old.
	const OWNABLE_ABI = [
	...
	for (const [label, addr] of [
		...
		try {
			...
			const owner = await ownable.getOwner().catch(() => ownable.owner());
			let pending = null;
			try {
