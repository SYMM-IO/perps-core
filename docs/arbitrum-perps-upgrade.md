...
4. Re-read the Safe's existing AccountLayer role-administration authority after both cuts. Separately inspect each configured PartyB. When the Safe does not yet hold PartyB `DEFAULT_ADMIN_ROLE` and `MANAGER_ROLE`, write the exact handoff calls to `report.json` under `externalActions.partyBAuthority`; the prior PartyB admin must execute those calls before the Safe batch can be generated.
5. Export or propose any remaining Safe Core authority action (currently `FEE_ADMIN_ROLE`), then export an independent PartyB wiring batch. For every configured PartyB it grants the new InstantLayer `TRUSTED_ROLE`, enables the PartyB multicast whitelist entry, registers the PartyB on the new InstantLayer (which also grants `OPERATOR_ROLE`), and grants the new InstantLayer `INSTANT_LAYER_ROLE` on AccountLayer. The task re-reads all five states before proceeding. It then executes the remaining new-layer and GaslessLayer wiring batch. Each continuation recomputes only the missing actions from chain state.
...
6. Export independent PartyB and replacement-wiring Safe files. PartyB wiring grants the clean InstantLayer the required PartyB trust, multicast, registration/operator, and AccountLayer `INSTANT_LAYER_ROLE` state. Replacement wiring grants the clean InstantLayer Core `INSTANT_LAYER_ROLE`, grants AccountLayer `SIGNER_SETTER_ROLE`, grants the clean GaslessLayer AccountLayer `ACCOUNT_CREATOR_ROLE`, grants it InstantLayer `OPERATOR_ROLE`, registers it as a Core operational-fee charger, and routes fees to the reviewed treasury.
