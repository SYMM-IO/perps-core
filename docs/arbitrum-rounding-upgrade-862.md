...
This release applies the bounded uPNL rounding fix to the existing Arbitrum Core at `0x573310dB6d160B26026B8706EBe9831c7dEF1D09`. It deploys four libraries and four facets. Each new facet address **ends in `862`**, using the existing CREATE2 factory `0x99B425BC19F99a1B922664c0E4fa8A0870CE9975`.
...
Use the dedicated local checkout prepared for the release:
...
3. Choose the contract deployment signer. At the reviewed Arbitrum block `501976277`, the factory's sole `DEPLOYER_ROLE` holder was `0xf12239317e985f6772F86407608B166EfA3E2f05`. Select its key; inspection checks the role again before rehearsal or live deployment. Unlock the appropriate Hardhat keystore key when prompted. The recipe uses `RPC_ARBITRUM` and `ETHERSCAN_APIKEY`; the selected signer supplies the deployment key.
...
5. After the rehearsal passes, type the displayed `DEPLOY version_0.8.6.2 ON 42161` phrase to authorize deployment. The task deploys the eight contracts and publishes them on Arbiscan.
...
If interrupted, continue the same task. The deployment journal reconciles submitted transactions and reuses confirmed creations. An uncertain transaction must be resolved before another deployment. Each fork retry uses a fresh attempt namespace. A pending Safe export is recomputed from chain state on continuation.
...
"create2": {
  "factory": {
    "mode": "reuse",
    "address": "0x99B425BC19F99a1B922664c0E4fa8A0870CE9975"
  },
  "groups": {
...
Local checks cover the rounding boundaries, batch position counts, eight-contract deployment, facet suffixes, linked-library parity, transaction-free resume, selector preservation, unexpected selector drift, and raw Safe calldata. A passing local test is not live upgrade evidence. The registered workflow requires its own successful Arbitrum fork rehearsal before any live deployment.
