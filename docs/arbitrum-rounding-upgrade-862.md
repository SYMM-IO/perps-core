...
7. Run `./symmio` in the same release checkout and choose **Continue active task**. The task checks the exact selector map, linked runtime bytecode, new getter and publication records. If Core is globally paused, it checks the multisig's `UNPAUSER_ROLE`, simulates `unpauseGlobal()` with read-only `eth_call`, prints a separate Safe Transaction Builder JSON path and waits.
8. Import that second JSON into the same Safe, check the Core target, zero ETH value and `unpauseGlobal()` calldata, then execute it. Choose **Continue active task** again. The task rechecks the installed upgrade and verifies the global flag is cleared before marking the release complete. If Core was already globally unpaused, no second transaction is needed.
...
The first Safe transaction contains only `diamondCut`, with a zero initializer and empty initialization calldata. It preserves the pause flags. The script then verifies the installed upgrade and generates a separate `unpauseGlobal()` JSON if needed. Core becomes globally unpaused when the multisig executes that second transaction; generating the JSON does not change chain state. `unpauseGlobal()` clears only the global flag; separately paused operations remain paused. Core ownership or factory administration alone does not grant `UNPAUSER_ROLE`. If the reviewed multisig lacks that role, export stops with the required role and address in the error.

The report records the second file at `unpauseDelivery.builderPath` and its state inspection at `unpause`. Continuing a pending export recomputes the required action from live state. The unpause file has a separate intent key and digest from the Core cut, so it cannot replace the reviewed cut file.
