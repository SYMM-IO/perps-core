/**
 * Final wiring/state verification for AccountLayer + InstantLayer.
 *
 * This is the post-cut wiring gate. It does not compare local bytecode to
 * deployed bytecode, and it does not publish source/ABI to a block explorer.
 *
 * Hardhat script that checks:
...
type Runtime = {
	connection: { networkName: string }
	ethers: any
	loadDeploymentState: <T extends object>(filePath: string, expected?: DeploymentStateMetadata) => T
	resolveConfigFile: (name: string, networkName: string, override?: string) => string
}
...
function loadVerifierDeploymentState<T extends object>(runtime: Runtime, filePath: string, expected: DeploymentStateMetadata): T {
	try {
		return runtime.loadDeploymentState<T>(filePath, expected)
	} catch (error) {
		if (error instanceof Error && error.message.includes("is missing metadata")) {
			console.warn(`WARNING: ${filePath} is missing metadata; using legacy output for read-only verification.`)
...
function summarizeCallError(error: unknown): string {
	const message = error instanceof Error ? error.message : String(error)
	if (message.includes("Diamond: Function does not exist")) return "Diamond: Function does not exist"
	return message.split("\n")[0] ?? message
}

async function loadConfig(runtime: Runtime): Promise<Config> {
	const { connection, ethers, loadDeploymentState, resolveConfigFile } = runtime
...
async function runWiringVerification() {
	const connectionModule = await import("../../test/helpers/hardhat-connection.js")
	const deploymentStateModule = await import("./utils/deploymentState.js")
	const sharedConfigModule = await import("./utils/sharedConfig.js")

	const runtime: Runtime = {
		connection: connectionModule.default,
		ethers: connectionModule.ethers,
		loadDeploymentState: deploymentStateModule.loadDeploymentState,
		resolveConfigFile: sharedConfigModule.resolveConfigFile,
	}
	...
	try {
		...
		results.push({
			name: "Core: AL is global hook",
			pass: false,
			detail: `getAffiliateHook(address(0)) failed: ${summarizeCallError(error)}`,
		})
	}
