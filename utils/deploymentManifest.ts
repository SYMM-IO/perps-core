...
type EnsureLibrariesArgs = {
	...
	getFactory?: (spec: DeploymentSpec, addresses: Record<string, string>) => Promise<any>
	deploy?: (name: string, factory: any) => Promise<{ address: string; contract?: any }>
...
export async function ensureLibraries({
	...
	for (const name of requiredLibraryOrder(scope, requiredLibraries)) {
		...
		const spec = getLibrarySpec(scope, name)
		const factory = getFactory ? await getFactory(spec, addresses) : await getLinkedContractFactory(ethers, scope, spec, addresses)
		let contract: any | undefined
		let address: string
		if (deploy) {
			const result = await deploy(name, factory)
