...
import { ensureLibraries, getFacetSpec, getLinkedContractFactory, linkedLibrariesFor, type DiamondScope } from "../../utils/deploymentManifest.js"
...
export async function updateFacet(args: FacetUpdateArgs): Promise<FacetUpdateReport> {
	...
	if (!facetAddress) {
		state.libraries = await ensureLibraries({
			...
			getFactory: async (librarySpec, addresses) => {
				const artifact = await hre.artifacts.readArtifact(librarySpec.artifact)
				const libraries = linkedLibrariesFor(args.scope, librarySpec, addresses)
